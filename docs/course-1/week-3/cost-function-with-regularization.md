---
summary: '<p>How to turn &quot;keep the parameters small&quot; into an actual objective gradient descent can minimize.</p><span class="toc-summary__sub">Key ideas</span><p class="toc-summary__chips">add a penalty · regularization parameter · don&#x27;t · fit the data · keep weights small · trade-off</p><span class="toc-summary__sub">Key formulas</span><div class="toc-summary__math"><span class="arithmatex">\(\min_{\vec{w},b}\ \underbrace{\frac{1}{2m}\sum_{i=1}^{m}\big(f(\vec{x}^{(i)}) - y^{(i)}\big)^2}_{\text{fit the data}} \;+\; 1000\,w_3^2 + 1000\,w_4^2\)</span><span class="arithmatex">\(J(\vec{w},b) = \frac{1}{2m}\sum_{i=1}^{m}\big(f(\vec{x}^{(i)}) - y^{(i)}\big)^2 \;+\; \frac{\lambda}{2m}\sum_{j=1}^{n} w_j^2\)</span></div>'
---
