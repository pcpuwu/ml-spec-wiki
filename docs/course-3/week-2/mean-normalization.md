---
summary: '<p>Just as feature normalization sped up linear regression in Course 1, mean normalization makes the recommender run faster and — more importantly — behave far more sensi…</p><span class="toc-summary__sub">Key ideas</span><p class="toc-summary__chips">mean normalization · every · average rating · mean-centered · new users · columns</p><span class="toc-summary__sub">Key formula</span><div class="toc-summary__math"><span class="arithmatex">\(\text{prediction for user } j \text{ on movie } i = \vec{w}^{(j)}\cdot\vec{x}^{(i)} + b^{(j)} + \mu_i\)</span></div>'
---
# Mean Normalization
